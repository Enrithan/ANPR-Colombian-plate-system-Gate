...
## 2024-06-11 - Replace chained replace calls with translate for string cleaning
**Learning:** Python string methods like `.replace()` create intermediate objects each time they are called. When chaining `.replace()` calls or using it in a loop for character removal, performance drops due to the multiple passes over the string and the constant creation of string objects. `str.translate` coupled with a pre-computed `str.maketrans` table does this character removal in a single C-level pass.
**Action:** When stripping multiple characters from short strings in tight loops (e.g., OCR post-processing), always initialize a translation table (`str.maketrans`) in the class constructor and use `.translate()` to replace multiple chained `.replace()` calls or `for` loops.
