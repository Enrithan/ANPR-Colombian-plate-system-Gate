...
## 2024-05-26 - Optimize YOLO inference properties access
**Learning:** Accessing bounding box properties (`box.xyxy[0]`, `box.conf[0]`, `box.cls[0]`) individually in a loop over YOLO `results.boxes` causes severe performance bottlenecks due to repeated synchronous CPU-GPU data transfers.
**Action:** Extract all bounding box data simultaneously by converting the entire tensor object to a NumPy array (`results.boxes.data.cpu().numpy()`), then loop over the native NumPy array to eliminate the sync cost. Additionally, use O(1) Python sets for filtering detection classes.
