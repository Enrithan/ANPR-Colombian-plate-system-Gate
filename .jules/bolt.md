...
**Learning:** String validation and formatting inside tight loops (like OCR post-processing) should avoid iterative string concatenation and list-based character checks.
**Action:** Use pre-computed `frozenset` collections for O(1) lookups, dictionary `.get()` for safe mapping, and `"".join()` for string construction.
