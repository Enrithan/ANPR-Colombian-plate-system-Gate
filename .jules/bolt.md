...
## 2025-03-21 - Optimize OpenCV Object and Array Allocations in High-Frequency Inference
**Learning:** In tight computer vision loops (e.g., executing OCR validation per frame), dynamically instantiating static NumPy arrays (like coordinate vectors) and initializing OpenCV objects (like CLAHE or convolution kernels) introduces unnecessary overhead. Profiling showed that moving these to module-level constants reduces allocation times significantly over hundreds of loop iterations.
**Action:** Identify unchanging parameters and configurations in computer vision workflows and hoist their instantiation to module-level precomputed constants to ensure they only allocate memory once upon application launch.
