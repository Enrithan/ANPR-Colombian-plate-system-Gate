...
## 2026-05-21 - Pre-computing Static Arrays for High-Frequency Loops
**Learning:** In high-frequency computer vision loops (e.g., OCR inference or perspective transforms), instantiating static NumPy arrays (like perspective transform coordinates or convolution kernels) introduces severe allocation overhead.
**Action:** Always pre-compute and cache these arrays as module-level constants to optimize performance. This is safe because OpenCV functions like `cv2.getPerspectiveTransform` and `cv2.filter2D` do not mutate their array arguments.
