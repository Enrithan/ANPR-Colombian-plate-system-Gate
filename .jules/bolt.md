...
## 2025-05-18 - Optimize OCR Text Normalization
**Learning:** License plate normalization across readers consistently removes the character set [' ', '-', '.', '_', '|']. Benchmarks show chained `.replace()` calls are up to 4x faster than looping over characters and calling `.replace()` on short strings. In CPython 3.12, `str.replace` returns the original string object if no replacement occurs, which minimizes allocations in chained sequences.
**Action:** Use chained replaces instead of character loops or translation tables in performance-sensitive OCR paths for string normalization.
