...
## 2024-05-24 - Vectorized YOLO Data Extraction
**Learning:** Iterating over PyTorch tensors (`results.boxes`) and accessing elements like `box.xyxy[0].tolist()` within a loop causes severe performance degradation due to repeated synchronous GPU-CPU data transfers and intermediate object allocation.
**Action:** Always extract bounding box data natively as a vectorized batch using `results.boxes.data.cpu().numpy()`, but remember to explicitly cast values back to native Python types (`float`, `int`) when appending to detection lists to prevent downstream serialization/tracker type errors.
