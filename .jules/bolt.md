...
## 2025-03-22 - Optimize High-Frequency NumPy Array Instantiations
**Learning:** Instantiating static NumPy arrays (like `dst` perspective transform coordinates or convolution kernels) inside high-frequency functions (e.g., OCR inference applied to multiple crops per frame) introduces severe memory allocation overhead. Benchmarks show a 60x speedup when these are pre-computed (0.27s vs 0.004s per 100k calls).
**Action:** Always pre-compute and cache static `np.array` objects as module-level constants (e.g., `_DST_ARRAY`) when used within tight inner loops.
