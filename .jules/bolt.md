...
**Learning:** String validation and formatting inside tight loops (like OCR post-processing in `util.py`) shouldn't use iterative string concatenation (`+=`) or list-based character checks (`dict.keys()`). These operations create unnecessary object overhead and quadratic delays per character.
**Action:** Always use pre-computed `frozenset` collections for O(1) character lookups, dictionary `.get()` for safe mapping without view allocation, and `"".join()` instead of `+=` for constructing result strings in OCR pipelines.
