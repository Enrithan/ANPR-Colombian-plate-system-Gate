...
## 2025-03-22 - Optimize YOLO Inference Loop
**Learning:** Accessing YOLO bounding box properties (e.g., `box.xyxy[0]`) inside a loop causes repeated synchronous GPU-CPU data transfers, which severely impacts performance. Additionally, checking class membership against a list causes O(N) lookup overhead.
**Action:** Extract all bounding box data at once using vectorized tensor operations like `results.boxes.data.cpu().numpy()` to minimize sync transfers. Use Python sets for class membership checks for O(1) lookup in high-frequency detection loops.
