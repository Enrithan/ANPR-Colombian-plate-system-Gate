...
## 2024-05-26 - Avoid OpenCV Constant Instantiation in Global Scope
**Learning:** Pre-computing static NumPy arrays as global constants saves allocation overhead. However, attempting to globally instantiate stateful OpenCV objects like `cv2.createCLAHE` can introduce severe thread-safety regressions and crashes in multi-threaded processing loops.
**Action:** Always restrict OpenCV stateful objects to local function scope, while static numpy arrays like perspective matrices can be safely lifted to the module level.
