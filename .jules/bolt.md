...
## 2026-05-20 - Pre-computing static Numpy Arrays
**Learning:** In high-frequency computer vision loops (e.g., OCR inference), instantiating static NumPy arrays (like perspective transform coordinates or convolution kernels) introduces severe allocation overhead. This can take ~0.22s for 100k iterations versus ~0.006s when pre-computed.
**Action:** Always pre-compute and cache static arrays as module-level constants to optimize performance. This is safe because OpenCV functions like cv2.getPerspectiveTransform and cv2.filter2D do not mutate their array arguments.
