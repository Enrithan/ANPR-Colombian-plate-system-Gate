...
## 2025-05-18 - Avoid repeated GPU-CPU transfers in detection loops
**Learning:** Iterating through `results.boxes` and accessing attributes like `box.xyxy[0]` individually inside a loop causes repeated, synchronous memory transfers from the GPU to the CPU, severely slowing down PyTorch inference loops (like per-frame object detection in YOLO).
**Action:** Extract all bounding box data at once using vectorized tensor operations (e.g., `results.boxes.data.cpu().numpy()`) before the loop. Be sure to cast the resulting NumPy scalar types back to native Python types (`float`, `int`) when unpacking to prevent downstream serialization or compatibility issues.
