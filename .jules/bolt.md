...
## 2026-06-11 - Precompute Static NumPy Arrays for CV Loops
**Learning:** In high-frequency computer vision loops (e.g., OCR inference or perspective transforms), instantiating static NumPy arrays (like destination coordinate arrays or filter kernels) dynamically introduces significant allocation overhead. These arrays remain constant and their recreation wastes time per frame.
**Action:** Always pre-compute and cache static NumPy arrays as module-level constants to optimize performance. OpenCV functions like `cv2.getPerspectiveTransform` and `cv2.filter2D` do not mutate their array arguments, making this optimization completely safe and highly effective.
