...
## 2024-06-27 - Pre-compute Static OpenCV Objects and Numpy Arrays in High-Frequency Paths
**Learning:** Instantiating static `np.array` objects (like perspective transform coordinates or convolution kernels) and OpenCV objects (like `cv2.createCLAHE`) inside tight computer vision loops (e.g., per-frame OCR inference) introduces measurable allocation overhead.
**Action:** Always pre-compute and cache these as module-level constants to optimize performance. This is safe because OpenCV functions generally do not mutate these argument objects.
