...
## 2024-06-10 - Vectorize YOLO bounding box extraction
**Learning:** When processing YOLO detection results, avoid accessing bounding box properties (e.g., `box.xyxy[0]`) inside a loop, as this causes repeated synchronous GPU-CPU data transfers.
**Action:** Instead, extract all bounding box data at once using vectorized tensor operations like `results.boxes.data.cpu().numpy()` to improve performance. Also use Python sets instead of lists for O(1) class membership checks.
