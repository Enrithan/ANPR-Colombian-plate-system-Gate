...
## 2025-06-13 - Extract Static NumPy Arrays in CV Loops
**Learning:** In high-frequency computer vision loops (like OCR inference where geometric transforms and filtering run per frame), instantiating static NumPy arrays (like perspective transform coordinates or convolution kernels) introduces severe allocation overhead.
**Action:** Always pre-compute and cache these arrays as module-level constants to optimize performance. OpenCV functions like `cv2.getPerspectiveTransform` and `cv2.filter2D` do not mutate their array arguments, so this is completely safe.
