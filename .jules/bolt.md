...
## 2025-03-22 - Precomputing Static OpenCV and NumPy Objects in High-Frequency Loops
**Learning:** In high-frequency computer vision loops (e.g., OCR inference running 30+ FPS), instantiating static NumPy arrays (like perspective transform coordinates or convolution kernels) and OpenCV objects (like `cv2.createCLAHE`) introduces measurable allocation overhead.
**Action:** Always pre-compute and cache these as module-level constants to optimize performance. This is safe because OpenCV functions do not mutate these arguments, avoiding concurrency issues in multi-threaded environments.
