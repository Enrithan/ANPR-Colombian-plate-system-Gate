...
## 2024-05-24 - YOLO GPU-CPU Transfer Bottleneck
**Learning:** Accessing bounding box properties iteratively (e.g., `box.xyxy[0].tolist()`) in a loop over YOLO `results.boxes` causes severe performance degradation due to repeated synchronous GPU-to-CPU data transfers for each box.
**Action:** Always extract the entire bounding box tensor at once using `results.boxes.data.cpu().numpy()` and iterate over the resulting numpy array. Also, use `set` instead of `list` for class ID lookups to ensure O(1) performance in the detection loop.
