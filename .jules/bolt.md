...
## 2024-05-26 - Optimize Substring Replacements
**Learning:** For multiple character removals (e.g., cleaning OCR text like `-`, `.`, etc.), utilizing `str.translate` initialized with `str.maketrans` in the class constructor is significantly faster and cleaner compared to chained `.replace()` calls or looping `.replace()` operations in tight paths.
**Action:** Always pre-compute a translation table and use `str.translate` when cleaning known sets of specific characters out of short strings in high-frequency execution pathways.
