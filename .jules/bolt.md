...
**Learning:** Iterating directly over `results.boxes` in Ultralytics YOLO triggers slow, synchronous GPU-CPU data transfers for every individual bounding box due to property access slicing internal tensors.
**Action:** Always extract bounding box data entirely at once using vectorized tensor operations like `results.boxes.data.cpu().numpy()` and iterate through the resulting numpy array to avoid GPU bottlenecks in detection loops. Ensure to cast numpy values back to native python types (float, int) to avoid serialization bugs downstream.
