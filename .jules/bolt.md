...
## 2024-05-26 - Optimize string cleaning in tight loops
**Learning:** In Python, doing multiple string character replacements sequentially via chained `replace()` calls or loops over `.replace()` is less performant and conceptually uglier than using a built-in precomputed lookup table with `.translate()`.
**Action:** Replace looped `replace()` calls with `str.translate()` initialized once via `str.maketrans()` inside classes parsing noisy textual data, like OCR post-processors.
