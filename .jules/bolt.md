...
## 2024-05-26 - Optimize OCR String Post-Processing Tight Loops
**Learning:** In tight OCR post-processing loops (like validating license plates per frame), constructing strings via iterative concatenation (`str += char`) and checking characters against generated lists (`c in ['0', '1', ...]`) or dictionary views (`c in dict.keys()`) creates significant overhead due to constant object allocation and O(N) lookups.
**Action:** Use module-level `frozenset` for O(1) character lookups, use `in dict` directly instead of `dict.keys()`, and use list accumulation with `"".join()` to build strings efficiently.
