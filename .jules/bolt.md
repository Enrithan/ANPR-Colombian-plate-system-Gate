...
## 2025-07-02 - Avoid synchronous GPU-CPU transfers in tight loops
**Learning:** Iterating over PyTorch or YOLO result tensors and accessing properties per element (e.g., `box.xyxy[0]`) causes repeated synchronous data transfers between the GPU and CPU, significantly bottlenecking high-frequency loops like frame-by-frame object detection.
**Action:** Extract all required tensor data at once using vectorized operations (e.g., `results.boxes.data.cpu().numpy()`) before entering iteration loops.
