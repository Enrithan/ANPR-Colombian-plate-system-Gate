...
## 2025-05-23 - Static NumPy Array Allocation Overhead
**Learning:** Instantiating static NumPy arrays (like perspective transform coordinates or convolution kernels) inside high-frequency computer vision loops (e.g., OCR inference or per-frame processing) introduces severe allocation overhead.
**Action:** Always pre-compute and cache these arrays as module-level constants to optimize performance. OpenCV functions like `cv2.getPerspectiveTransform` and `cv2.filter2D` do not mutate their array arguments, making this safe.
