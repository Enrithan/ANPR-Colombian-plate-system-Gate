...
def license_complies_format(text):
    ...
    for i in range(3, 5):
        if not (text[i] in VALID_DIGITS or text[i] in dict_char_to_int):
            return False

    # Last 1 can be a number (Car) OR a letter (Motorcycle)
    last_char_valid_number = text[5] in VALID_DIGITS or text[5] in dict_char_to_int
...
def format_license(text):
    ...
    if text[5] in VALID_LETTERS and text[5] not in VALID_DIGITS:
        is_motorcycle = True
