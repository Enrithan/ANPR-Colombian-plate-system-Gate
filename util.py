...
# ⚡ Bolt: Pre-computed sets for O(1) string validation
...
def license_complies_format(text):
    ...
    # ⚡ Bolt: Use O(1) set lookup for character validation instead of list/dict.keys() search
...
def format_license(text):
    ...
    if text[5] in string.ascii_uppercase and text[5] not in '0123456789':
        is_motorcycle = True
    ...
    # ⚡ Bolt: Construct list instead of iterative string concatenation (+=)
    # and use dict.get(key, key) for safe and faster fallback lookups
    res = []
    for j, c in enumerate(text):
        if j < 3: # Always letters
            res.append(dict_int_to_char.get(c, c))
        elif j < 5: # Always numbers
            res.append(dict_char_to_int.get(c, c))
