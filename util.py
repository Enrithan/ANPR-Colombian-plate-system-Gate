...
# Pre-computed sets for O(1) character lookup inside loops
...
def license_complies_format(text):
    ...
    last_char = text[5]
    if last_char in VALID_NUMBERS or last_char in VALID_LETTERS:
        return True
    return False
...
def format_license(text):
    ...
    if last_char in string.ascii_uppercase and not last_char.isdigit():
        is_motorcycle = True
    ...
    for j, c in enumerate(text):
        if j < 3: # Always letters
            parts.append(dict_int_to_char.get(c, c))
        elif j < 5: # Always numbers
            parts.append(dict_char_to_int.get(c, c))
