...
# Pre-computed sets for O(1) lookups to optimize string validation in tight loops
...
_VALID_CARS_LETTERS = _VALID_LETTERS | _DICT_INT_TO_CHAR_KEYS
_VALID_CARS_NUMBERS = _VALID_NUMBERS | _DICT_CHAR_TO_INT_KEYS
...
def license_complies_format(text):
    ...
    if text[0] not in _VALID_CARS_LETTERS or \
       text[1] not in _VALID_CARS_LETTERS or \
       text[2] not in _VALID_CARS_LETTERS:
        return False

    # Next 2 are ALWAYS numbers
    if text[3] not in _VALID_CARS_NUMBERS or \
       text[4] not in _VALID_CARS_NUMBERS:
        return False

    # Last 1 can be a number (Car) OR a letter (Motorcycle)
    last_c = text[5]
    if last_c in _VALID_CARS_NUMBERS or last_c in _VALID_CARS_LETTERS:
        return True
...
def format_license(text):
    ...
    # OR if it's uniquely mapped to a letter going backwards (e.g. '0' trying to be an 'O', dict_int_to_char has '0':'O')
    is_motorcycle = (last_c in _VALID_LETTERS and last_c not in _VALID_NUMBERS) or \
                    (last_c in _DICT_INT_TO_CHAR_KEYS and last_c not in _DICT_CHAR_TO_INT_KEYS)

    # Use dict.get() for safer mapping and a list compression to avoid string concatenation overhead in loops
    return "".join([
        ...
        dict_int_to_char.get(last_c, last_c) if is_motorcycle else dict_char_to_int.get(last_c, last_c)
    ])
