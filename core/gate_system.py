...
class GateEntrySystem:
    ...
    # ⚡ Bolt: Combined regex for faster evaluation
    COLOMBIAN_PATTERN_COMBINED = re.compile(
        r'^('
        r'[A-Z]{3}[0-9]{3}|'       # LLL-NNN (Private)
        r'[A-Z]{3}[0-9]{2}[A-Z]|'  # LLL-NNA (Motorcycle)
        r'[A-Z]{2}[0-9]{4}'        # LL-NNNN (Older)
        r')$'
    )
    ...
    def _validate_pattern(self, plate_text: str) -> bool:
        ...
        # ⚡ Bolt: Use the combined regex pattern instead of looping through multiple
