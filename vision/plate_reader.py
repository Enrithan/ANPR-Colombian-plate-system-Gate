...
# Pre-compute arrays for perspective transform and sharpening kernel to avoid overhead in tight loops
_DST_ARRAY = np.array([
    [0, 0],
    [320 - 1, 0],
    [320 - 1, 160 - 1],
...
class EasyOCRPlateReader(IPlateReader):
    ...
    def correct_perspective(self, img):
        ...
        # ⚡ Bolt: Use pre-computed _DST_ARRAY for performance
        M = cv2.getPerspectiveTransform(rect, _DST_ARRAY)
    ...
    def read_text(self, cropped_plate: np.ndarray) -> tuple[str, float]:
        ...
        # ⚡ Bolt: Use pre-computed _SHARPEN_KERNEL
        ...
        for detection in detections:
            ...
            # ⚡ Bolt: Chained replace instead of list iteration for performance
