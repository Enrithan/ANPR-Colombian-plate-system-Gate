...
# Pre-compute static numpy arrays for performance in tight loops
_DST_PTS = np.array([
    [0, 0],
    [320 - 1, 0],
    [320 - 1, 160 - 1],
...
_SHARPENING_KERNEL = np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]])
...
class EasyOCRPlateReader(IPlateReader):
    ...
    def read_text(self, cropped_plate: np.ndarray) -> tuple[str, float]:
        ...
        sharpened = cv2.filter2D(contrast_enhanced, -1, _SHARPENING_KERNEL)
