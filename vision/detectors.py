...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            # ⚡ Bolt: Use a set for O(1) membership lookups in detection loop
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Extract all box data at once to avoid GPU-CPU sync overhead per box
            boxes_data = results.boxes.data.cpu().numpy()
            for row in boxes_data:
                # Cast to native python types to avoid downstream serialization issues
                cls = int(row[5])
