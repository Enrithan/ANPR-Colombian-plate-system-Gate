...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            # ⚡ Bolt: Convert list to set for O(1) lookups during detection loop
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Avoid accessing properties on individual box objects inside the loop
            # as it triggers slow GPU-CPU synchronization per property.
            # Extract all data to CPU memory in one vectorized operation.
