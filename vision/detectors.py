...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            # ⚡ Bolt: Convert list to set for O(1) membership checks during detection
            self.vehicle_classes = {2, 3, 5, 7}
        else:
            # ⚡ Bolt: Ensure custom classes are also O(1) sets
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Vectorize GPU->CPU transfer to avoid multiple synchronization overheads
            boxes_data = results.boxes.data.cpu().numpy()
            for row in boxes_data:
                cls = int(row[5])
                if cls in self.vehicle_classes:
                    # row: [x1, y1, x2, y2, conf, cls]
                    # Ensure coordinates are native floats to prevent downstream JSON serialization errors
