...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            self.vehicle_classes = {2, 3, 5, 7} # ⚡ Bolt: Use set for O(1) membership lookup
        else:
            self.vehicle_classes = set(vehicle_classes) # ⚡ Bolt: Convert to set
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Extract all data at once to avoid sync GPU-CPU transfers in a loop
