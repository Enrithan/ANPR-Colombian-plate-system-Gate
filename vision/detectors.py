...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            # ⚡ Bolt: Store classes as a set for O(1) lookups in tight loops
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Extract all box data at once via vectorized CPU transfer
            # This avoids repeated synchronous GPU-CPU syncs in the loop
