...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            # ⚡ Bolt: Use a set for O(1) membership lookups instead of list iteration
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Extract all bounding boxes at once via a vectorized tensor copy.
            # Iterating `results.boxes` property-by-property (e.g. `box.xyxy[0]`) causes
            # repeated synchronous GPU-CPU memory transfers, stalling the pipeline.
            boxes_data = results.boxes.data.cpu().numpy()
            for row in boxes_data:
                # Structure: [x1, y1, x2, y2, conf, cls]
                cls = int(row[5])
                if cls in self.vehicle_classes:
                    # Explicitly cast to native types for downstream compatibility
                    detections.append([
