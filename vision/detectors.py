...
class YOLOVehicleDetector(IVehicleDetector):
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Extract all bounding box data at once using vectorized numpy ops
            # instead of box.xyxy[0] to prevent synchronous GPU-CPU data transfers inside the loop.
            data = results.boxes.data.cpu().numpy()
            for row in data:
                # row structure: [x1, y1, x2, y2, conf, cls]
                cls = int(row[5])
                if cls in self.vehicle_classes:
                    # Explicit cast to native Python types (float) to avoid downstream serialization bugs
