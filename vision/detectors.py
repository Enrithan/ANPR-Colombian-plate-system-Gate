...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        # ⚡ Bolt: Use a set for vehicle_classes to ensure O(1) lookups
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Extract all bounding boxes at once to avoid repeated synchronous GPU-CPU data transfers inside the loop.
            boxes_data = results.boxes.data.cpu().numpy()
            for row in boxes_data:
                # results.boxes.data contains [x1, y1, x2, y2, conf, cls]
