...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            self.vehicle_classes = {2, 3, 5, 7}  # ⚡ Bolt: Use set for O(1) lookups
        else:
            self.vehicle_classes = set(vehicle_classes)  # ⚡ Bolt: Use set for O(1) lookups
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Extract all bounding box data at once using vectorized tensor operations
            # to avoid repeated synchronous GPU-CPU data transfers inside the loop
            boxes_data = results.boxes.data.cpu().numpy()
            for row in boxes_data:
                cls = int(row[5])
                if cls in self.vehicle_classes:
                    # ⚡ Bolt: Explicitly cast the extracted numpy values back to native Python types
