...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            self.vehicle_classes = {2, 3, 5, 7}  # ⚡ Bolt: O(1) set lookup
        else:
            self.vehicle_classes = set(vehicle_classes)  # ⚡ Bolt: O(1) set lookup
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Extract all box data at once to avoid sync GPU-CPU transfers in the loop.
            # Cast elements to native Python types for consistency.
