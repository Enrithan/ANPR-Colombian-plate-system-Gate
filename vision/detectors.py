...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            # ⚡ Bolt: Use a set for O(1) lookups during high-frequency detection loops
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Extract all bounding box data at once with a vectorized tensor operation.
            # This prevents repeated synchronous GPU-CPU data transfers inside the loop.
            boxes_data = results.boxes.data.cpu().numpy()
            for row in boxes_data:
                cls = int(row[5])
                if cls in self.vehicle_classes:
                    # Explicitly cast extracted numpy values back to native Python types
                    # to ensure type consistency and avoid downstream serialization bugs.
                    detections.append([
