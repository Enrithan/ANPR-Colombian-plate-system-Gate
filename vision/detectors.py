...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            # ⚡ Bolt: Use sets instead of lists for O(1) lookup in detection loops
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        if results.boxes:
            ...
            # Accessing properties like box.xyxy[0] inside a loop causes slow GPU-CPU data transfers.
            boxes_data = results.boxes.data.cpu().numpy()
            for row in boxes_data:
                # Cast back to native python types to prevent downstream serialization bugs
                cls = int(row[5])
