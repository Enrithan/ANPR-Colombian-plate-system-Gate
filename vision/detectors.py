...
class YOLOVehicleDetector(IVehicleDetector):
    def __init__(self, model_path: str, vehicle_classes: list = None):
        if vehicle_classes is None:
            self.vehicle_classes = {2, 3, 5, 7} # ⚡ Bolt: Use set for O(1) membership lookups
        else:
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Vectorized extraction prevents synchronous GPU-CPU transfers per box
            # and avoids creating intermediate PyTorch tensor objects in loop.
            data = results.boxes.data.cpu().numpy()
            for row in data:
                # Cast the values out of numpy explicitly (using float/int) to ensure
                # compatibility with downstream tracking which expects native Python types.
                if int(row[5]) in self.vehicle_classes:
