...
class YOLOVehicleDetector(IVehicleDetector):
    ...
    def detect(self, frame: np.ndarray) -> list:
        ...
        detections = []
        if results.boxes:
            # ⚡ Bolt: Vectorized box data extraction to prevent repeated synchronous GPU-CPU transfers
            boxes_data = results.boxes.data.cpu().numpy()
            for row in boxes_data:
                x1, y1, x2, y2, conf, cls = row
                if int(cls) in self.vehicle_classes:
                    # Explicit cast to python primitives for downstream serialization and type safety
